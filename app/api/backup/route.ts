...
export async function GET(request: Request) {
  try {
    ...
    return new Response(JSON.stringify({ exportedAt: nowIso(), farm: 'Ali Livestock', records: records.results, users: users.results, files: files.results, audit: audit.results }, null, 2), {
      headers: { 'Content-Type': 'application/json; charset=utf-8', 'Content-Disposition': `attachment; filename="ali-livestock-backup-${nowIso().slice(0,10)}.json"`, 'Cache-Control': 'no-store' },
    });
  } catch (error) {
