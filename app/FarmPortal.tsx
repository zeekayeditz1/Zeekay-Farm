...
function RecordForm({module,config,onClose,onSaved}:{module:string;config:ModuleConfig;onClose:()=>void;onSaved:()=>void}){
  ...
  useEffect(()=>{if(module==='breeding'&&form.matingDate&&!form.expectedCalvingDate){const d=new Date(form.matingDate);d.setDate(d.getDate()+283);setForm(prev=>({...prev,expectedCalvingDate:d.toISOString().slice(0,10)}));}},[module,form.matingDate,form.expectedCalvingDate]);
